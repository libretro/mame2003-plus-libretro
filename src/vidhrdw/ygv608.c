...
static void get_tile_info_A_8( int offset )
{
	...
	else
	{
		...
		{
			log_cb(RETRO_LOG_DEBUG, LOGPRE  "A_8X8: tilemap=%d\n", j );
			j = 0;
		}
...
static void get_tile_info_B_8( int offset )
{
	...
	else
	{
		...
		if( j >= Machine->drv->gfxdecodeinfo[set].gfxlayout->total )
		{
			log_cb(RETRO_LOG_DEBUG, LOGPRE  "B_8X8: tilemap=%d\n", j );
			j = 0;
		}
...
static void get_tile_info_A_16( int offset )
{
  ...
  if( col >= ygv608.page_x ) {
    ...
    if( j >= Machine->drv->gfxdecodeinfo[set].gfxlayout->total ) {
	log_cb(RETRO_LOG_DEBUG, LOGPRE  "A_16X16: tilemap=%d\n", j );
      j = 0;
    }
...
static void get_tile_info_B_16( int offset )
{
  ...
  if( col >= ygv608.page_x ) {
    ...
    if( j >= Machine->drv->gfxdecodeinfo[set].gfxlayout->total ) {
	log_cb(RETRO_LOG_DEBUG, LOGPRE  "B_16X16: tilemap=%d\n", j );
      j = 0;
    }
  ...
  for( i=0; i<YGV608_MAX_SPRITES; i++, sa-- )
  {
    ...
    case SZ_8X8 :
      ...
      if( code >= Machine->drv->gfxdecodeinfo[GFX_8X8_4BIT].gfxlayout->total ) {
	    log_cb(RETRO_LOG_DEBUG, LOGPRE  "SZ_8X8: sprite=%d\n", code );
	    code = 0;
      }
	...
	color = ( code >> ( (spf + 1) * 2 ) ) & 0x0f;
	  ...
	  log_cb(RETRO_LOG_DEBUG, LOGPRE  "SZ_32X32: sprite=%d\n", code );
	code = 0;
      }
    ...
    case SZ_64X64 :
      ...
      if( code >= Machine->drv->gfxdecodeinfo[GFX_64X64_4BIT].gfxlayout->total ) {
	    log_cb(RETRO_LOG_DEBUG, LOGPRE  "SZ_64X64: sprite=%d\n", code );
	    code = 0;
      }
	...
	switch (offset)
	{
		...
		case 0x04: /* P#4 - register data port */
		{
			...
			if (ygv608.ports.s.p5 & p5_rrai)
			{
				...
				{
					regNum = 0;
	   				log_cb(RETRO_LOG_DEBUG, LOGPRE  "warning: rn=50 after read increment\n" );
				}
		...
		default :
			log_cb(RETRO_LOG_DEBUG, LOGPRE  "unknown ygv608 register (%d)\n", offset );
			break;
	}
...
WRITE16_HANDLER( ygv608_w )
{
	...
	switch (offset)
	{
		...
		case 0x04: /* P#4 - register data port */
		{
			UINT8 regNum = (ygv608.ports.s.p5) & p5_rn;
#if 0
			log_cb(RETRO_LOG_DEBUG, LOGPRE  "R#%d = $%02X\n", regNum, data );
#endif
			...
			if (ygv608.ports.s.p5 & p5_rwai)
			{
				...
				{
					regNum = 0;
	    			log_cb(RETRO_LOG_DEBUG, LOGPRE  "warning: rn=50 after write increment\n" );
				}
				ygv608.ports.s.p5 &= ~p5_rn;
				ygv608.ports.s.p5 |= regNum;
			}
...
void HandleRomTransfers( void )
{
  ...
  int bytes = (int)ygv608.regs.s.tn4 << 4;
    ...
    log_cb(RETRO_LOG_DEBUG, LOGPRE  "Transferring data from rom...\n" );
...
void nvsram( offs_t offset, data16_t data )
{
  ...
  if( 1 ) {
    ...
    if( i%16 == 0 )
      log_cb(RETRO_LOG_DEBUG, LOGPRE  "%04X: ", offset );
    log_cb(RETRO_LOG_DEBUG, LOGPRE  "%02X ", data );
    ascii[i%16] = ( isprint( data ) ? data : '.' );
    if( i%16 == 15 )
      log_cb(RETRO_LOG_DEBUG, LOGPRE  "| %-16.16s\n", ascii );
  }
...
void SetPostShortcuts( int reg )
{
	...
	switch (reg)
	{
		...
		case 6:
#if 0
			log_cb(RETRO_LOG_DEBUG, LOGPRE  "SBA = $%08X\n", (int)ygv608.regs.s.sba << 13 );
#endif
			break;
  ...
  case 9 :
    ...
    /*    log_cb(RETRO_LOG_DEBUG, LOGPRE  "SLH = %1X\n", ygv608.regs.s.slh );*/
    break;
...
void dump_block( char *name, unsigned char *block, int len )
{
  ...
  log_cb(RETRO_LOG_DEBUG, LOGPRE  "unsigned char %s[] = {\n", name );
  for( i=0; i<len; i++ ) {
    if( i%8 == 0 )
      log_cb(RETRO_LOG_DEBUG, LOGPRE  " " );
    log_cb(RETRO_LOG_DEBUG, LOGPRE  "0x%02X, ", block[i] );
    if( i%8 == 7 )
      log_cb(RETRO_LOG_DEBUG, LOGPRE  "\n" );
  }
  log_cb(RETRO_LOG_DEBUG, LOGPRE  "};\n" );
...
READ16_HANDLER( debug_trigger )
{
   ...
   */
#if 1
  log_cb(RETRO_LOG_DEBUG, LOGPRE  "Pattern Name Table\n" );
  for( i=0; i<4096; i++ ) {
    if( i % 16 == 0 )
      log_cb(RETRO_LOG_DEBUG, LOGPRE  "$%04X : ", i );
    log_cb(RETRO_LOG_DEBUG, LOGPRE  "%02X ", ygv608.pattern_name_table[i] );
  ...
  log_cb(RETRO_LOG_DEBUG, LOGPRE  "Scroll Table\n" );
  for( i=0; i<512; i++ ) {
    if( i % 16 == 0 )
      log_cb(RETRO_LOG_DEBUG, LOGPRE  "$%04X : ", i );
    log_cb(RETRO_LOG_DEBUG, LOGPRE  "%02X ", ygv608.scroll_data_table[0][i] );
...
void ShowYGV608Registers( void )
{
  ...
  log_cb(RETRO_LOG_DEBUG, LOGPRE  "YGV608 Registers\n" );
  logerror(
  ...
  for( p=0; p<2; p++ ) {
    log_cb(RETRO_LOG_DEBUG, LOGPRE  "\t" );
    for( b=0; b<8; b++ ) {
      log_cb(RETRO_LOG_DEBUG, LOGPRE  "%02X ", ygv608.base_addr[p][b] );
    }
    log_cb(RETRO_LOG_DEBUG, LOGPRE  "\n" );
  }
}
