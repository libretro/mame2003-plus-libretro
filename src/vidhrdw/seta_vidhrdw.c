				...
				if (new_bank != seta_samples_bank)
				{
					...
					if (samples_len == 0x240000)	/* blandia, eightfrc */
					{
						...
						if ( (samples_len > 0x100000) && ((addr+0x40000) <= samples_len) )
							...
							log_cb(RETRO_LOG_DEBUG, LOGPRE "PC %06X - Invalid samples bank %02X !\n", activecpu_get_pc(), new_bank);
					}
...
PALETTE_INIT( usclssic )
{
	...
	for (x = 0x000; x < 0x200 ; x++)
	{
		...
		if (x>=0x100) palette_set_color(x,pal5bit(data >> 10),pal5bit(data >> 5),pal5bit(data >> 0));
		else palette_set_color(x+0x300,pal5bit(data >> 10),pal5bit(data >> 5),pal5bit(data >> 0));
	}
...
static void zombraid_drawcrosshairs( struct mame_bitmap *bitmap, const struct rectangle *cliprect )
{
	...
	draw_crosshair(1, bitmap,p1_x,p1_y,cliprect);
	draw_crosshair(2, bitmap,p2_x,p2_y,cliprect);
}
