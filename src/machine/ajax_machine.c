...
READ_HANDLER( ajax_ls138_f10_r )
{
	...
	switch ((offset & 0x01c0) >> 6){
		...
		default:
			log_cb(RETRO_LOG_ERROR, LOGPRE "%04x: (ls138_f10) read from an unknown address %02x\n",activecpu_get_pc(), offset);
	}
...
WRITE_HANDLER( ajax_ls138_f10_w )
{
	switch ((offset & 0x01c0) >> 6){
		...
		default:
			log_cb(RETRO_LOG_ERROR, LOGPRE "%04x: (ls138_f10) write %02x to an unknown address %02x\n",activecpu_get_pc(), data, offset);
	}
}
