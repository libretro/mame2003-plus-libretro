...
static READ32_HANDLER( f3_control_r )
{
	...
	log_cb(RETRO_LOG_ERROR, LOGPRE "CPU #0 PC %06x: warning - read unmapped control address %06x\n",activecpu_get_pc(),offset);
	return 0xffffffff;
}
...
static WRITE32_HANDLER( f3_control_w )
{
	...
	log_cb(RETRO_LOG_ERROR, LOGPRE "CPU #0 PC %06x: warning - write unmapped control address %06x %08x\n",activecpu_get_pc(),offset,data);
}
...
static WRITE32_HANDLER( f3_sound_bankswitch_w )
{
	if (f3_game==KIRAMEKI) {
		...
		log_cb(RETRO_LOG_ERROR, LOGPRE "Sound bankswitch in unsupported game\n");
	}
}
...
GAMEC( 1995, bubblem,  0,        f3_224a, f3, bubblem,  ROT0,   "Taito Corporation Japan",   "Bubble Memories - The Story Of Bubble Bobble 3 (World)", &generic_btn_label, &bubblem_bootstrap )
