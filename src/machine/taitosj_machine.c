...
{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: protection read\n",activecpu_get_pc());
#endif
	return 0;
}
...
{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: protection write %02x\n",activecpu_get_pc(),data);
#endif
}
...
{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: protection status read\n",activecpu_get_pc());
#endif
	return 0xff;
}
...
{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: protection read %02x\n",activecpu_get_pc(),toz80);
#endif
	zaccept = 1;
	return toz80;
}
...
{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 port A read %02x\n",activecpu_get_pc(),portA_in);
#endif
	return portA_in;
}
...
{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 port A write %02x\n",activecpu_get_pc(),data);
#endif
	portA_out = data;
}
...
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 port B write %02x\n",activecpu_get_pc(),data);
	...
	{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705  68INTRQ **NOT SUPPORTED**!\n",activecpu_get_pc());
#endif
	}
	if (~data & 0x02)
	{
		...
		portA_in = fromz80;
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 <- Z80 %02x\n",activecpu_get_pc(),portA_in);
#endif
	}
	if (~data & 0x04)
	{
...
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 -> Z80 %02x\n",activecpu_get_pc(),portA_out);
	...
	{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 write %02x to address %04x\n",activecpu_get_pc(),portA_out,address);
#endif
	...
	{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 read %02x from address %04x\n",activecpu_get_pc(),portA_in,address);
#endif
        memory_set_context(0);
	...
	{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 address low %02x\n",activecpu_get_pc(),portA_out);
#endif
	...
	{
#if DEBUG_MCU
log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 68705 address high %02x\n",activecpu_get_pc(),portA_out);
#endif
...
READ_HANDLER( spacecr_prot_r )
{
	...
	if( pc != 0x368A && pc != 0x36A6 )
		log_cb(RETRO_LOG_DEBUG, LOGPRE "Read protection from an unknown location: %04X\n",pc);
