...
READ_HANDLER( wow_speech_r )
{
	...
	log_cb(RETRO_LOG_DEBUG, LOGPRE "Data : %d Speech : %s at intonation %d\n",Phoneme, PhonemeTable[Phoneme],Intonation);

	if(Phoneme==63) {
   		sample_stop(wowChannel);
				log_cb(RETRO_LOG_DEBUG, LOGPRE "Clearing sample %s\n",totalword);
	...
	for (i=0; wowWordTable[i]; i++) {
	  ...
	  /* WOW has Dungeon */
	     ...
	     log_cb(RETRO_LOG_DEBUG, LOGPRE "Storing sample position %d and copying string %s\n",plural,oldword);
		  ...
		  sample_set_freq(wowChannel, wowBaseFrequency);         /* play at correct rate */
		  log_cb(RETRO_LOG_DEBUG, LOGPRE "Playing sample %d\n",i);
		  totalword[0] = 0;				   /* Clear the total word stack */
		  return data;
	   }
	}
...
{
	log_cb(RETRO_LOG_DEBUG, LOGPRE "asked for samples status %d\n",wowChannel);
