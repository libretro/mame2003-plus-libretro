...
WRITE32_HANDLER( policetr_video_w )
{
	...
	if (mem_mask)
		log_cb(RETRO_LOG_DEBUG, LOGPRE "%08X: policetr_video_w access with mask %08X\n", activecpu_get_previouspc(), ~mem_mask);
	...
	switch (offset)
	{
		...
		case 2:
		{
			switch (video_latch)
			{
				...
				/* log anything else */
				default:
					log_cb(RETRO_LOG_DEBUG, LOGPRE "%08X: policetr_video_w(2) = %08X & %08X with latch %02X\n", activecpu_get_previouspc(), data, ~mem_mask, video_latch);
					break;
			}
			break;
		}
			...
			switch (video_latch)
			{
				...
				case 0x00:
					if (data != (0 << 20) && data != (1 << 20) && data != (2 << 20))
						log_cb(RETRO_LOG_DEBUG, LOGPRE "%08X: policetr_video_w(3) = %08X & %08X with latch %02X\n", activecpu_get_previouspc(), data, ~mem_mask, video_latch);
					break;
...
READ32_HANDLER( policetr_video_r )
{
	...
	/* log anything else */
	log_cb(RETRO_LOG_DEBUG, LOGPRE "%08X: policetr_video_r with latch %02X\n", activecpu_get_previouspc(), video_latch);
	return 0;
}
