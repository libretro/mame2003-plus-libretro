...
static READ_HANDLER( thunderx_bankedram_r )
{
	...
	else if (rambank & 0x10)
	{
		if (pmcbank)
		{
/*			log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x read pmcram %04x\n",activecpu_get_pc(),offset);*/
			return pmcram[offset];
		}
		else
		{
			log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x read pmc internal ram %04x\n",activecpu_get_pc(),offset);
			return 0;
		}
	}
	else
...
/*			if (offset == 0x200)	debug_signal_breakpoint(1);*/
		if (pmcbank)
		{
			log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x pmcram %04x = %02x\n",activecpu_get_pc(),offset,data);
			pmcram[offset] = data;
		}
		else
...
{
/* log_cb(RETRO_LOG_DEBUG, LOGPRE "%04x: 1f98_w %02x\n",activecpu_get_pc(),data);*/
...
/*	log_cb(RETRO_LOG_DEBUG, LOGPRE "thunderx %04x: bank select %02x\n", activecpu_get_pc(), lines );*/
