...
READ16_HANDLER( jaguar_jerry_regs_r )
{
	if (offset != JINTCTRL && offset != JINTCTRL+2)
		log_cb(RETRO_LOG_DEBUG, LOGPRE "%08X:jerry read register @ F10%03X\n", activecpu_get_previouspc(), offset * 2);
...
WRITE16_HANDLER( jaguar_jerry_regs_w )
{
	...
	if (offset != JINTCTRL && offset != JINTCTRL+2 && offset != ASICTRL)
		log_cb(RETRO_LOG_DEBUG, LOGPRE "%08X:jerry write register @ F10%03X = %04X\n", activecpu_get_previouspc(), offset * 2, data);
}
...
{
	log_cb(RETRO_LOG_DEBUG, LOGPRE "%08X:jaguar_serial_r(%X)\n", activecpu_get_previouspc(), offset);
	return 0;
}
...
WRITE32_HANDLER( jaguar_serial_w )
{
	switch (offset)
	{
		...
		case 5:
			if ((data & 0x3f) != 0x15)
				log_cb(RETRO_LOG_DEBUG, LOGPRE "Unexpected write to SMODE = %X\n", data);
			if ((data & 0x3f) == 0x15)
			{
		...
		default:
			log_cb(RETRO_LOG_DEBUG, LOGPRE "%08X:jaguar_serial_w(%X,%X)\n", activecpu_get_previouspc(), offset, data);
			break;
	}
}
