...
static INLINE int GET_KEYSCALED_RATE(int rate, int keycode, int keyscale)
{
...
static INLINE int GET_INTERNAL_KEYCODE(int block, int fns)
{
	int n43;
	if (fns < 0x780)
	{
...
static INLINE int GET_EXTERNAL_KEYCODE(int block, int fns)
{
...
static INLINE void calculate_step(YMF271Slot *slot)
{
	double st;
...
static INLINE void update_lfo(YMF271Slot *slot)
{
...
static INLINE int calculate_slot_volume(YMF271Slot *slot)
{
	UINT64 volume;
	UINT64 env_volume;
...
static void update_pcm(YMF271Chip *chip, int slotnum, INT32 *mixp, int length)
{
	...
	if (slot->waveform != 7)
	{
		printf("Waveform %d in update_pcm !!!", slot->waveform);
	}
...
// <--------|
// +--[S1]--+--[S3]-->
static INLINE INT32 calculate_2op_fm_0(YMF271Chip *chip, int slotnum1, int slotnum2)
{
...
// <-----------------|
// +--[S1]--+--[S3]--|-->
static INLINE INT32 calculate_2op_fm_1(YMF271Chip *chip, int slotnum1, int slotnum2)
{
	YMF271Slot *slot1 = &chip->slots[slotnum1];
	YMF271Slot *slot2 = &chip->slots[slotnum2];
	INT64 env1, env2;
...
static INLINE INT32 calculate_1op_fm_0(YMF271Chip *chip, int slotnum, int phase_modulation)
{
	YMF271Slot *slot = &chip->slots[slotnum];
	INT64 env;
	INT64 slot_output;
...
// <--------|
// +--[S1]--|
static INLINE INT32 calculate_1op_fm_1(YMF271Chip *chip, int slotnum)
{
	YMF271Slot *slot = &chip->slots[slotnum];
	INT64 env;
	INT64 slot_output;
...
static void ymf271_update(int num, INT16 **outputs, int length)
{
	int i, j;
	int op;
	INT32 *mixp;
...
static void ymf271_timer_a_tick(int chipnum)
{
...
static void ymf271_timer_b_tick(int chipnum)
{
	YMF271Chip *chip = &YMF271[chipnum]; 
...
static void ymf271_init(int chipnum, UINT8 *rom, void (*cb)(int), read8_handler ext_read, write8_handler ext_write)
{
	YMF271Chip *chip = &YMF271[chipnum];
	memset(&YMF271[chipnum], 0, sizeof(YMF271Chip));
	

	chip->timA = timer_alloc(ymf271_timer_a_tick);
	chip->timB = timer_alloc(ymf271_timer_b_tick);
...
int YMF271_sh_start( const struct MachineSound *msound )
{
	...
	for(i=0; i<intf->num; i++)
	{
		...
		stream_init_multi(2, name, vol, Machine->sample_rate, i, ymf271_update);
	}
