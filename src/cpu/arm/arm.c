...
/*	if (rn==0xf) log_cb(RETRO_LOG_ERROR, LOGPRE "%08x:  Source R15\n",R15);*/
	...
	if (insn & INSN_SDT_L)
	{
		...
		else
		{
			...
			{
				if (ARM_DEBUG_CORE)
					log_cb(RETRO_LOG_ERROR, LOGPRE "%08x:  LDR to R15\n",R15);
	...
	if (!(insn & INSN_SDT_P)/* && (insn&INSN_SDT_W)*/)
	{
		...
		else
		{
			...
			if (rd==rn) {
				SetRegister(rn,GetRegister(rd));
			/*	log_cb(RETRO_LOG_ERROR, LOGPRE "Arm %08x: LDR style with rn==rn\n",R15);*/
			}
			else {
				SetRegister(rn,(rnv - off));
...
static void HandleALU( data32_t insn )
{
	...
	if ((opcode & 0xc) != 0x8)
	{
		if (rdn == eR15 && !(insn & INSN_S))
		{
			...
			if (ARM_DEBUG_CORE)
				log_cb(RETRO_LOG_ERROR, LOGPRE "%08x:  Suspected R15 mode change\n",R15);
		}
		else
		{
			if (rdn==eR15) {
				if (ARM_DEBUG_CORE)
					log_cb(RETRO_LOG_ERROR, LOGPRE "%08x: Setting R15 with S flag\n",R15);
