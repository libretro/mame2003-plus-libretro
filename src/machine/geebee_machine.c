...
READ_HANDLER( geebee_in_r )
{
	...
	log_cb(RETRO_LOG_ERROR, LOGPRE "in_r %d $%02X\n", offset & 3, data);
	return data;
}
...
WRITE_HANDLER( geebee_out6_w )
{
    ...
    {
	case 0:
		log_cb(RETRO_LOG_ERROR, LOGPRE "out6_w:0 ball_h   $%02X\n", data);
        geebee_ball_h = data ^ 0xff;
        break;
    case 1:
		log_cb(RETRO_LOG_ERROR, LOGPRE "out6_w:1 ball_v   $%02X\n", data);
        geebee_ball_v = data ^ 0xff;
        break;
    case 2:
		log_cb(RETRO_LOG_ERROR, LOGPRE "out6_w:2 n/c      $%02X\n", data);
#ifdef MAME_DEBUG
        ...
        break;
    default:
		log_cb(RETRO_LOG_ERROR, LOGPRE "out6_w:3 sound    $%02X\n", data);
...
WRITE_HANDLER( geebee_out7_w )
{
	...
	case 4:
		...
		break;
	case 5:
		log_cb(RETRO_LOG_ERROR, LOGPRE "out7_w:5 bgw      $%02X\n", data);
		geebee_bgw = data & 1;
		break;
	case 6:
		log_cb(RETRO_LOG_ERROR, LOGPRE "out7_w:6 ball on  $%02X\n", data);
		geebee_ball_on = data & 1;
		break;
	case 7:
		log_cb(RETRO_LOG_ERROR, LOGPRE "out7_w:7 inv      $%02X\n", data);
