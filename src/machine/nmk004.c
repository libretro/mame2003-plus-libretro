...
static int NMK004_sound_init_hack = 0;
...
static void oki_play_sample(int sample_no)
{
	...
	else
	{
		...
		if (sample != 0)
		{
			UINT8 *rom = memory_region((chip == 0) ? REGION_SOUND1 : REGION_SOUND2);
...
static void real_nmk004_init(void)
{
	...
	NMK004_sound_init_hack = 1;
}


WRITE16_HANDLER( NMK004_w )
{
	if (NMK004_sound_init_hack == 0) {
		real_nmk004_init();
	}
