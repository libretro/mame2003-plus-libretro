...
static WRITE16_HANDLER( unk880000_w )
{
	...
	switch( offset )
	{
		...
		case 0x22/2:
			...
			watchdog_reset_w(0,0); // hmm not sure
			{
			   cpu_set_nmi_line(1, PULSE_LINE);
			}
...
static READ16_HANDLER( unk880000_r )
{
	...
	//	if ( video_screen_get_vpos(machine->primary_screen) >= 240) return 0;
			if ( cpu_getscanline() >= 240) return 0; // correct.??
			else return 1;
...
//	sprite_bitmap = auto_bitmap_alloc(320,240,BITMAP_FORMAT_INDEXED16);
	sprite_bitmap = auto_bitmap_alloc_depth(320,240,16); // this is correct i think
...
static void tecmosys_do_final_mix(struct mame_bitmap* bitmap)
{
